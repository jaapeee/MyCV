...
<div class="container">
	<div class="form" style="width: 50%;  height: 680px; float: left; background: #a9a9a9;">
    	<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" style="margin-top: -100px;">
			...
			<center><span class="error" style="color:maroon;"> <?php echo $passwordErr;?></span><br></center>
        	<label for="snum"><h2>Email Address : </h2></label>
        		<input  type="text" name="stnum" required size="35" placeholder="Default: sampleemail@gmail.com"><br>

			
			<center><span class="error" style="color:maroon;"> <?php echo $passwordErr;?></span><br></center>
        	<label for="spass"><h2>Password :</h2></label>
        		<input  type="password" name="pword" required size="35" placeholder="Default: 123123">
            
            <h3 class="back"><a href="../index.php">Back</a></h3>
            <h1 class="student">Faculty Login</h1>
...
if(isset($_POST['sign'])){
  ...
  //query
  $result = mysql_query("select * from personalinformation where emailaddress = '".$stnum."' and password = '".$password."' and uid = '0' ")
  ...
  //Login Process
  if(($row['emailaddress'] == $_SESSION["stnumber"]) && ($row['password'] == $_SESSION["pwd"])){
      echo "<script>alert('Login Successfully!');</script>";
      echo "<script>location.href = 'faculty-home.php'</script>";
  }  else {
